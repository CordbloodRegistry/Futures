//
//  cbrContactsCreateViewControllerViewController.m
//  fieldMobile
//
//  Created by Remina Sangil on 5/2/12.
//  Copyright (c) 2012 __MyCompanyName__. All rights reserved.
//

#import "cbrContactsCreateViewControllerViewController.h"

@interface cbrContactsCreateViewControllerViewController ()

@end

@implementation cbrContactsCreateViewControllerViewController
...
- (void)viewDidLoad
{
    ...
    if (self.detailItem != nil) {
        ...
        if ([entityName isEqualToString:@"Contacts"]) {	
            ...
            if ([[[self.detailItem valueForKey:@"kol"] description] isEqualToString:@"Y"])
                self.kol.selected = true;
            else
                self.kol.selected = false;
...
- (IBAction)saveRecord:(id)sender {
    ...
    if([self.role.text length] == 0)
    {
        errorMsg = [NSString stringWithFormat:@"%@%@\n",errorMsg,@"Role is required"];
    }
    if([self.continuum.text length] == 0)
    {
        errorMsg = [NSString stringWithFormat:@"%@%@\n",errorMsg,@"Continuum is required"];
    }
    if([self.role.text isEqualToString:@"Primary Kit Contact"])
    {
    ...
    else 
    {
        ...
        if ([[[self.detailItem entity] name] isEqualToString:@"Contacts"])
        {
            ...
            if (self.kol.isOn) {
                [self.detailItem setValue:@"Y" forKey:@"kol"];
            }
            else {
                [self.detailItem setValue:@"N" forKey:@"kol"];
            }
            ...
            [newContact setValue:self.continuum.text forKey:@"continuum"];
            if (self.kol.isOn) {
                [newContact setValue:@"Y" forKey:@"kol"];
            }
            else {
                [newContact setValue:@"N" forKey:@"kol"];
            }
            ...
            {
                [self.detailItem setValue:@"" forKey:@"kitContactId"];
                [self.detailItem setValue:[newContact valueForKey:@"firstName"] forKey:@"kitContactFirstName"];
                [self.detailItem setValue:[newContact valueForKey:@"lastName"] forKey:@"kitContactLastName"];
