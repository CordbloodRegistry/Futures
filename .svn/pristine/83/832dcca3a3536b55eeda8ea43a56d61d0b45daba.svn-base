...
#define KEY_TMK_AES 0x10
#define KEY_DUKPT_MASTER 0x20
...
/**
 ...
 @return TRUE if function succeeded, FALSE otherwise
 **/
-(BOOL)barcodeEngineSetInitString:(NSString *)data error:(NSError **)error;
 ...
 **/
-(NSString *)barcodeEngineGetInitString:(NSError **)error;
...
/**
 ...
 @return TRUE if function succeeded, FALSE otherwise
 */
-(BOOL)btmGetEnabled:(BOOL *)enabled error:(NSError **)error;
...
/**
 ...
 @return TRUE if function succeeded, FALSE otherwise
 */
-(BOOL)btmSetEnabled:(BOOL)enabled error:(NSError **)error;
...
/**
 ...
 @return TRUE if function succeeded, FALSE otherwise
 */
-(BOOL)btmWrite:(void *)data length:(int)length error:(NSError **)error;
...
/**
 ...
 @return TRUE if function succeeded, FALSE otherwise
 */
-(BOOL)btmWrite:(NSString *)data error:(NSError **)error;
 ...
 */
-(int)btmRead:(void *)data length:(int)length timeout:(double)timeout error:(NSError **)error;
 ...
 */
-(NSString *)btmReadLine:(double)timeout error:(NSError **)error;
 ...
 */
-(NSString *)btmGetLocalName:(NSError **)error;
...
/**
 ...
 @param maxTime the max time to discover, in seconds. Actual time may vary.
 @param error pointer to NSError object, where error information is stored in case function fails. You can pass nil if you don't want that information
 @return array of strings containing bluetooth device address and bluetooth device name, i.e. if 2 printers are found, the list will contain @"address 1",@"name 1",@"address 2",@"name 2" if function succeeded, nil otherwise
 */
-(NSArray *)prnDiscoverPrinters:(int)maxDevices maxTime:(double)maxTime error:(NSError **)error;
...
/**
 ...
 @return TRUE if function succeeded, FALSE otherwise
 */
-(BOOL)prnDiscoverPrintersInBackground:(int)maxDevices maxTime:(double)maxTime error:(NSError **)error;
...
/**
 Loads new key, encrypted with already loaded Terminal Master Key (TMK). The command is unavailable if the device is tampred.
 @param keyData an array, that consists of:
 1. When loading new TMK:
 - ENCRYPTION KEY ID - the ID of the already existing key, used to encrypt the new key data. Set it to KEY_TMK_AES (0x10)
 - KEY ID - the ID of the key to set, put KEY_TMK_AES (0x10)
 - KEY VERSION - the version of the key in high to low order, 4 bytes, cannot be 0
 - KEY - the key data, 16 bytes
 - HASH - SHA256 of the previous bytes (ENCRYPTION KEY ID, KEY ID, KEY VERSION, KEY and optionally DUKPT SERIAL)

 <br>KEY + HASH have to be put inside the packet encrypted with AES128 using key ENCRYPTION KEY. SHA256 is calculated on the unencrypted data. The head decrypts the data and then calculates and compares the hash. If the calculated SHA does not match the SHA sent with the command, the key excahnge is rejected and error is returned.

 2. When loading new DUKPT key:
 - MAGIC NUMBER - put 0x2A in here
 - ENCRYPTION KEY ID - the ID of the already existing key, used to encrypt the new key data. Set it to KEY_TMK_AES (0x10)
 - KEY ID - the ID of the key to set, put KEY_DUKPT (0x20)
 - KEY VERSION - the version of the key in high to low order, 4 bytes, cannot be 0
 - KEY - the key data, 16 bytes
 - DUKPT SERIAL - 10 bytes DUKPT initial serial number
 - PADDING - 6 bytes of 0x00, used to pad the data to 16 bytes
 - HASH - SHA256 of the previous bytes (MAGIC NUMBER, ENCRYPTION KEY ID, KEY ID, KEY VERSION, KEY, DUKPT SERIAL and PADDING)

 <br>KEY + DUKPT SERIAL + PADDING + HASH (2) have to be put inside the packet encrypted with AES128 using key ENCRYPTION KEY. SHA256 is calculated on the unencrypted data. The head decrypts the data and then calculates and compares the hash. If the calculated SHA does not match the SHA sent with the command, the key excahnge is rejected and error is returned.
 @param error pointer to NSError object, where error information is stored in case function fails. You can pass nil if you don't want that information
